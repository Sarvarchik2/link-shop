...
def create_order(order_create: OrderCreate, current_user: User = Depends(get_current_user)):
    with Session(engine) as session:
        ...
        db_order = Order(user_id=current_user.id, total_price=total_price, status="pending")
...
def get_admin_stats(current_user: User = Depends(get_current_admin)):
    with Session(engine) as session:
        ...
        return DashboardStats(total_sales=total_sales, total_orders=total_orders, total_users=total_users, total_products=total_products)
